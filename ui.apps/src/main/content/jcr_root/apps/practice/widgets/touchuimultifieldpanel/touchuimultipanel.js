...
(function () {
            ...
            function postProcess(data){
                ...
                _.each(mValues, function (record, i) {
                    ...
                    _.each(record, function(rValue, rKey){
                        ...
                        if($field.prop("type") != "checkbox" && _.isArray(rValue) && !_.isEmpty(rValue)){
                            fillNestedFields( $($fieldSets[i]).find("[data-init='multifield']"), rValue);
                        }else{
                            if($field.prop("type") == "checkbox"){
                                if(rValue && rValue.length > 0 && rValue[0] == "true")
                                	$field.prop("checked", rValue);
                            } else {
                                $field.val(rValue);
                            }
                        }
    ...
    function fillValue($field, record){
        ...
        if($field.prop("type") == "checkbox"){
            record[name] = $field.prop("checked") ? ["true"] : "";
        } else {
